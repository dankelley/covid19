...
for (region in regions) {
    ...
    if (!interactive()) png(paste0("covid19b_", region, ".png"),
                            width=6, height=6, unit="in", res=120, pointsize=11)
    ...
    mtext(sprintf("%s confirmed %d, deaths %d",
                  region,
    ...
    ## phase plot
    x <- confirmed$data
    dx <- oce::lowpass(diff(x) + 0.01, n=7)
    #lim <- c(1, max(c(x,dx)))
    lim <- c(1, 1e6)
    par(mar=c(3,3,1,1), mgp=c(2, 0.7, 0))
    plot(x[-1], dx, type="l", log="xy", axes=FALSE,
         xlim=lim, ylim=lim,
         xlab="Count", ylab="Daily change in count")
    box()
    oce::oceAxis(side=1, logStyle="decade")
    oce::oceAxis(side=3, logStyle="decade", labels=FALSE)
    oce::oceAxis(side=2, logStyle="decade")
    oce::oceAxis(side=4, logStyle="decade", labels=FALSE)
