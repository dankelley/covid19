...
class, so we can set aside the possibility that $n$ is 400 in large classes.
...
formula^[If $P\ll 1$ and $n$ is small, as is the present case, the formula may
be approximated as $nP$, which permits calculation by eye.] is the key to
inferring infection probability during a lecture.
...
are inferred from reported data for British Columbia on 2021-08-12, and caution
dictates these to be plausible estimates of the Nova Scotian situation a few
weeks into classes, if past is prologue^[A refinement to the present
calculation might break down Dalhousie students by point of origin, but this
is not information available to the author.].
...
It should be noted that the model used here is crude.  The province has access
to more sophisticated models, but I fear that that Dalhousie may be relying on
models calibrated to the Springtime conditions, which is clearly not relevant
today, with the Delta variant.  There is also a question as to whether models
developed for people passing by others in a mall are applicable to a classroom
setting.

Respect for the community dictates that Dalhousie should at least revisit
decisions it made months ago and, for the first time, document its
decision-making process publicly.
...
My honest response will be that I do not understand the Administration's choice
not to require vaccination, and cannot defend that choice.

# Commentary

Dalhousie has long taken pride in its contributions to the community. Will it
be proud to be the cause of unnecessary suffering in a province that has
