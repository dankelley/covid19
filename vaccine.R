...
for (location in c("Canada", "United Kingdom", "United States")) {
    ...
    if (nrow(dd)) {
        ...
        m <- lm(v100 ~ day)
        oce.plot.ts(dd$time, dd$total_vaccinations_per_hundred, xlab="", ylab="Vaccines/100", drawTimeRange=FALSE, grid=TRUE, xlim=c(vaccineStart, now), type="o")
        ...
        oce.plot.ts(dd$time, dd$total_vaccinations_per_hundred, xlab="", ylab="Vaccines/100", log="y", logStyle="decade", drawTimeRange=FALSE, grid=TRUE, xlim=c(vaccineStart, now), type="o")
